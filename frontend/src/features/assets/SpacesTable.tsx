...
export function SpacesTable<T extends Asset>({ spaces, onEdit, onDelete, extra, nameAddon }: { spaces: T[]; onEdit?: (s: T) => void; onDelete?: (s: T) => void; extra?: (s: T) => React.ReactNode; nameAddon?: (s: T) => React.ReactNode }) {
  return (
    <Table>
      ...
      <tbody>
        {spaces.map((s) => {
          ...
          return (
            <tr key={s.id}>
              <Td l={t("assets.spaceName")}><span className="inline-flex items-center gap-1"><span className="font-semibold">{s.name}</span>{nameAddon?.(s)}</span></Td>
